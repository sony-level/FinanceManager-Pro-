...
def treasury_dashboard(request):
    ...
    total_in = transactions.filter(amount__gt=0).aggregate(total=Sum('amount'))['total'] or 0
    total_out = transactions.filter(amount__lt=0).aggregate(total=Sum('amount'))['total'] or 0
...
def reconciliation_create(request):
    ...
    if not all([invoice_id, transaction_id, matched_amount]):
        return Response({'error': 'invoice_id, bank_transaction_id et matched_amount requis'}, status=400)
    ...
    try:
        transaction = BankTransaction.objects.get(id=transaction_id, entreprise=entreprise)
